...
class Tests:
                ...
                for key, value in params_dict.items():
                    if key in kwargs:
                        if not kwargs[key] in value:
                            raise ValueError(f"""{kwargs[key]} is not an allowed argument for the parameter {key}.
                            Valid values are {', '.join(value)}""")
                return obj
            return wrapper
        return validate
    ...
    def __init__(self,
        ...
        self.variants = np.setdiff1d(self.data[self.treatment_col].unique(), [self.control_variant_name])
    ...
    def extract_regression_results(self, reg_results: sm.regression.linear_model.OLSResults , param_index:int=1) -> Dict:
        ...
        if self.alternative == 'two-sided':
            ...
            conf_intervals = tuple(reg_results.conf_int(alpha=self.alpha).iloc[1 + param_index].values)
        else:
            ...
            conf_intervals = tuple(reg_results.conf_int(alpha=self.alpha * 2).iloc[1 + param_index].values)
    ...
    def ttest(self, data: pd.DataFrame, metric:str) -> Dict:
        if len(self.variants)==1:
            test = ttest_ind(data[data[self.treatment_col]==self.variants[0]][metric],
                                data[data[self.treatment_col]==self.control_variant_name][metric],
                                alternative = self.alternative)
            ate = data.groupby(self.treatment_col)[metric].mean().diff()[1]
            p_value = test[1]

            conf_intervals = test.confidence_interval(confidence_level = 1 - self.alpha)[0:2]
            ...
            raise ValueError(f'You have {len(self.variants)} variants but t-test is not a valid test for multivarint scenario.')
    ...
    def ols(self, data:pd.DataFrame, metric:str):
        ...
        if self.cluster_cols:
            regression_results = smf.ols(f"{metric} ~ {' + '.join(data.drop([metric] + self.cluster_cols, axis=1))}",
                                                 data = data).fit(cov_type='cluster',
                                                                  cov_kwds={'groups': data[self.cluster_cols]})
        else:
            regression_results = smf.ols(f"{metric} ~ {' + '.join(data.drop(metric, axis=1))}",
                                                 data = data).fit()
        for idx, variant in enumerate(self.variants):
            ...
            results_dict.update(self.extract_regression_results(regression_results, idx))
            yield results_dict

    
    def apply_correction(self, results: pd.DataFrame):

        adjusted = multipletests(pvals = results['p_value'],
                                         alpha = self.alpha,
